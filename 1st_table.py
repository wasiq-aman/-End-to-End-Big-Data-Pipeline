...
PG_TABLE = 'public.1st_table'

# CONFIG - MINIO
MINIO_ENDPOINT = 'http://127.0.0.1:9000'
MINIO_ACCESS_KEY = 'minio'
MINIO_SECRET_KEY = 'minio123'
MINIO_BUCKET = '1st_table'
...
def extract_day_data(target_date):
    ...
    q = f'''
        ...
        WHERE "RECORD_TIME"::date = '{target_date}'
...
def write_temp_parquet(df, target_date):
    ...
    temp_key = f"sensor_data/temp/{target_date}.parquet"
...
def save_to_monthly_parquet(df, target_date):
    ...
    key = f"sensor_data/{year}/{month_name}_raw.parquet"
...
def load_to_timescale(df):
    ...
    for dcol in ('RECORD_DATE', 'SHIFT_DAY', 'MODIFIED_DATE'):
